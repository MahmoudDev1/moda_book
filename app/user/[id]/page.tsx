    ...
    <div className="box bg-white shadow-sm p-3 rounded-md">
      <div className="flex justify-between flex-wrap gap-10 items-center">
        <div className="flex gap-2 items-start">
          ...
          />
          <div>
            <div className="mt-2 flex items-center gap-1 sm:gap-3 flex-wrap">
              <div className="font-semibold text-lg">{user.name}</div>
              {user.role == "admin" && (
                <div className="bg-sky-200 px-2 py-1 rounded-lg text-sky-700 flex gap-2 items-center text-sm">
                  <RiAdminFill />
                  Website Owner
                </div>
              )}
      ...
      {posts.length > 0 && (
        <div className="posts mt-8 grid grid-cols-1 sm:grid-cols-2 gap-2">
          {posts.map((post: Post) => {
            return post.image ? (
